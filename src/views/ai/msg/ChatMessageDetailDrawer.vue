<template>
  <a-drawer v-model:visible="visible" title="对话消息详情" :width="width >= 580 ? 580 : '100%'" :footer="false">
    <a-descriptions :column="2" size="large" class="general-description">
      ...
      <a-descriptions-item label="提问">{{ dataDetail?.question }}</a-descriptions-item>
      <a-descriptions-item label="回答">{{ dataDetail?.answer }}</a-descriptions-item>
      <a-descriptions-item label="模型名称">{{ dataDetail?.model }}</a-descriptions-item>
      <a-descriptions-item label="是否采纳,0:未点击.1:采纳.2,未采纳">{{ dataDetail?.adopt }}</a-descriptions-item>
